...
class Validator
{
    ...
    public function validate(\Magento\Customer\Api\Data\AddressInterface $address) 
    {
        ...
        // if ($address->getCountryId() === "US") {
            ...
            $client->withLicenses(["us-core-cloud"]);
        ...
        //         // $lookup->setAddress3((count($street)>2) ? $street[2] : "");
        //     }
        //     if ($region = $address->getRegion()) {
        ...
        try {
            ...
            if (empty($result)) {
                $response['message'] = __(
                    'Your address could not be validated. Please try to enter a valid address.'
                    //  If you continue to experience issues, please contact <a href="mailto:ecommsales@curvature.com">eCommSales@Curvature.com</a>'
                );
            } else {
                $response['valid'] = true;
